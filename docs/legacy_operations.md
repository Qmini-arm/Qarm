...
本文保存 Viser 统一重构前的 README，供核对历史实验和接口使用。下文的网页平台、
独立硬件控制命令、构建默认值和界面行为可能已经失效；当前入口和能力以
[项目 README](../README.md) 与 [当前架构](architecture.md) 为准。
历史硬件命令不属于新的 Viser 控制链路，不应按本文直接启用设备。

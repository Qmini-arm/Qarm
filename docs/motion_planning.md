...
`base_link → joint_1 ... joint_6 → tool0` 的六轴串联链，
并增加 `world → base_link` 的固定坐标变换。xacro 的六关节零位就是机械臂
`home_pose`。

home pose 下，`base_link` 中的 `tool0` 位置为
`[0.682063, 0.047818, -0.162527] m`；建模总质量约为 `3.6655 kg`。配置的重力在
`world` 中为 `[0, 0, -9.80665] m/s²`，经 xacro 的 `world_to_base` 旋转后，在
`base_link` 中约为 `[-9.806647, -0.005266, -0.005767] m/s²`。home pose 的末端→基座
方向与重力方向夹角约 `13.97°`，因此是接近共线，不是严格共线。
...
规划限位取 URDF `<limit>` 与 `<safety_controller>` 的交集。当前六轴软限位绝对值依次为
`[170°, 89.95°, 115°, 114.59°, 115°, 85.94°]`，不再是六轴共用同一限位。对应硬限位
绝对值为 `[180°, 100.27°, 150.11°, 120°, 120°, 89.95°]`，所以每个软限位都严格位于
...
5. “六轴 M8010 控制参数”表同步显示每台电机当下的计划命令；

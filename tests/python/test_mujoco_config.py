...
def test_joint_map_requires_fresh_four_axis_calibration() -> None:
    mapping = JointMap.load(ROOT / "config/joint_map.json")
    assert mapping.joint_names == tuple(f"joint_{i}" for i in range(1, 5))
    assert mapping.motor_ids_by_joint.tolist() == [0, 1, 2, 3]
    assert np.all(mapping.direction == 1)
    assert not mapping.zero_calibrated
    assert not mapping.direction_calibrated
    assert not mapping.calibrated


def test_gravity_deploy_config_matches_four_axis_template() -> None:
    ...
    assert int(values["schema_version"]) == 3
    assert values["calibration_confirmed"] == "false"
    ...
    assert np.allclose(
        ...
        [-3.141592654, -1.75, -2.62, -2.094395102],
    )
    assert np.allclose(
    ...
    assert values["expected_board_boot_id"] == "UNCALIBRATED"
    assert "calibration" not in mapping_raw
    ...
    assert np.allclose(floats("rotor_at_reference_rad"), np.zeros(4))
    pose = json.loads((ROOT / "config/calibration_pose.json").read_text())
    assert np.allclose(floats("reference_joint_rad"), pose["reference_joint_rad"])


@pytest.mark.parametrize("key", ["direction", "zero_offset_rad", "motor_ids_by_joint"])
def test_joint_map_rejects_stale_six_value_fields(tmp_path, key: str) -> None:
    raw = json.loads((ROOT / "config/joint_map.json").read_text())
    raw[key] = [0, 1, 2, 3, 4, 5]

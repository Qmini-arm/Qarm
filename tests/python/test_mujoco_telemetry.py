...
def test_parse_and_map_motor_order() -> None:
    ...
    mapping = JointMap(
        ...
        motor_ids_by_joint=np.array([0, 1, 2, 3]),
        ...
        zero_offset_rad=np.array([0, 0.5, 0, 0], dtype=float),
        calibrated=True,
    )
    state = map_joint_state(sample, mapping)
    assert np.allclose(state.position, [0, -0.5, 2, 3])
    assert np.allclose(state.velocity, [0, -0.1, 0.2, 0.3])
...
def test_reference_capture_survives_later_direction_change() -> None:
    ...
    source_at_reference = np.arange(4, dtype=float)
    mapping = JointMap(
        joint_names=tuple(f"joint_{i}" for i in range(1, 5)),
        motor_ids_by_joint=np.arange(4),
        angle_field="q_output_rad",
        velocity_field="dq_output_rad_s",
        torque_field="tau_ideal_output_nm",
        direction=np.array([-1, 1, -1, 1], dtype=float),
...
def test_four_axis_telemetry_follows_motor_ids_instead_of_array_indices() -> None:
    sample = TelemetrySample.from_line(_line())
    assert sample is not None
    mapping = replace(
        JointMap.load(),
        motor_ids_by_joint=np.array([3, 1, 0, 2]),
        direction=np.ones(4),
        zero_offset_rad=np.zeros(4),
        zero_calibrated=False,
    )
    state = map_joint_state(sample, mapping)
    assert np.allclose(state.position, [3, 1, 0, 2])
    assert np.allclose(state.velocity, [0.3, 0.1, 0, 0.2])
